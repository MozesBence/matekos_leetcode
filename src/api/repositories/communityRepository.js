      ...
      const postsWithBase64Files = await Promise.all(postsWithComments.map(async (postObj) => {
        ...
        for (const comment of postObj.Community_comments) {
          ...
          Comments.push({
            ...
            like: Number(comment.Community_likes[0]?.dataValues.total_likes) ?? 0,
            dislike: Number(comment.Community_likes[0]?.dataValues.total_dislikes) ?? 0,
            ...
            comments: comment.replies ? comment.replies.map(inner_comment => {
              ...
              return {
                ...
                like: Number(inner_comment.Community_likes[0]?.dataValues.total_likes) ?? 0,
                dislike: Number(inner_comment.Community_likes[0]?.dataValues.total_dislikes) ?? 0,
                prepareReply: false,
