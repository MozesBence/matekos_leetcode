...
const { Tasks,Users, User_customization, Notification } = db;
...
const tasksRepository = {
  ...
  async getTaskCount({ difficulty, search, themes, state, userId }) {
    try {
        ...
        const themeTaskIds = await this.getTasksByThemes(themes);
        const stateTaskIds = await this.getTaskByState({ state, userId });
        const finalTaskIds = await this.getFinallTaskIds(themeTaskIds, stateTaskIds);
        return await this.returnTaskCount(whereClause, finalTaskIds);

    } catch (error) {
        throw error;
    }
},
async returnTaskCount(whereClause, finalTaskIds) {
  try {
  ...
  async getFilteredTasks({ difficulty, search, themes, state, userId, offset }) {
    try {
        ...
        const stateTaskIds = await this.getTaskByState({ state, userId });
        const finalTaskIds = await this.getFinallTaskIds(themeTaskIds, stateTaskIds);
        return await this.returnFilteredTasks(whereClause, finalTaskIds, offset);
...
async returnFilteredTasks(whereClause, finalTaskIds, offset) {
  try {
      if (finalTaskIds !== null) {
          whereClause.id = { [Op.in]: finalTaskIds };
      }
...
async getFinallTaskIds(themeTaskIds,stateTaskIds){
...
async getTasksByThemes(themes) {
  try {
      ...
      return themesWithTasks.flatMap(theme => theme.Tasks.map(task => task.id));
...
async getTaskByState({ state, userId}){
